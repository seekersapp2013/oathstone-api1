...
const BNB_TESTNET_URL = process.env.BNB_TESTNET_URL; // BNB Testnet URL
const BNB_MAINNET_URL = process.env.BNB_MAINNET_URL; // BNB Mainnet URL
...
async function deployBNBContract(req, res) {
    try {
        ...
        res.status(500).json({ error: "Failed to deploy BNB contract." });
    }
}
